...
export function PropertyDetailModal({ property, onClose, onLocate }: PropertyDetailModalProps) {
  ...
  return (
    <>
      ...
      {/* Render Modals entirely outside the backdrop to prevent click bubbling */}
      <EditPropertyDialog
        property={
          property
            ? {
              ...property,
              location: (property as any).location || (property.lng && property.lat ? `POINT(${property.lng} ${property.lat})` : undefined)
            } as any
            : null
        }
        open={isEditOpen}
